...
library<-read.csv("C:/Users/Mahe/Downloads/activity")

activity$date<-as.POXITlt(activity$date,"%Y-%m-%d")
...
hist(activity_total_steps$steps,main="Total number of steps taken per day", xlab=
       "Total steps taken per day",col="darkblue",ylab=c(0,20),breaks=seq(0,25000,by=2500))
...
plot(average_daily_activity$interval,average_daily_activity$mean,type="l",col="darkblue",
...
hist(total_steps_imputed$daily_steps,col="darkblue",xlab = "Total steps per day",
     ylim=c(0,30),main="Total number of steps taken each day", breaks=seq(0,25000,by=2500))
